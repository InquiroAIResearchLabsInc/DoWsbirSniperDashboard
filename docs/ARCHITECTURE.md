# Architecture — DSIP Sniper · ART Edition
...
- **Empty state on Topics.** When zero SNIPER tier topics are visible, the API returns `empty_state: true` with the top 3 EVALUATEs and a banner from `docs/copy/empty_state.md`. The center panel is never blank as long as the DB has any opportunity.
